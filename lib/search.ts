...
const searchIds = async (
  ...
  const conditions = config.columns
    .map((col) => `unaccent(${col}) ILIKE unaccent($1)`)
    .join(" OR ")
...
export const searchEventIds = (search: string) =>
  searchIds(EVENT_SEARCH_COLUMNS, search)

export const searchNewsIds = (search: string) =>
  searchIds(NEWS_SEARCH_COLUMNS, search)
